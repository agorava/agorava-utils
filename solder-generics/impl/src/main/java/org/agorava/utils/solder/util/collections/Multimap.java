 ...
 * <p/>
 * <p/>
 * All methods that alter the multimap are optional, and the views returned by
 * the multimap may or may not be modifiable. When modification isn't supported,
...
public interface Multimap<K, V> {
    ...
    /**
     * Returns {@code true} if the multimap contains the specified key-value
     * pair.
     *
     * @param key   key to search for in multimap
     * @param value value to search for in multimap
    ...
    /**
     * Returns a collection, which may contain duplicates, of all keys. The
     * number of times of key appears in the returned multiset equals the number
     * of mappings the key has in the multimap. Changes to the returned multiset
     * will update the underlying multimap, and vice versa.
     *
     * @return a multiset with keys corresponding to the distinct keys of the
     *         multimap and frequencies corresponding to the number of values
     *         that each key maps to
     */
    Multiset<K> keys();

    /**
     * Returns a collection of all values in the multimap. Changes to the
     * returned collection will update the underlying multimap, and vice versa.
     *
     ...
     * collection will update the underlying multimap, and vice versa. The
     * entries collection does not support the {@code add} or {@code addAll}
     * operations.
     *
     ...
     * will update the underlying multimap. The map does not support {@code
     ...
     * <p/>
     * <p/>
     * In general, two multimaps with identical key-value mappings may or may not
     * be equal, depending on the implementation. For example, two
     * {@link SetMultimap} instances with the same key-value mappings are equal,
     * but equality of two ListMultimap instances depends on the ordering of the
     * values for each key.
     * <p/>
     * <p/>
     * A non-empty {@link SetMultimap} cannot be equal to a non-empty
     * ListMultimap, since their {@link #asMap} views contain unequal
